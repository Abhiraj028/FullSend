...
export function ConnectionDialog({ sender, senderNickname, onAccept, onReject }: ConnectionDialogProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-xl border border-slate-200 shadow-lg p-6 w-80 space-y-4">
        ...
        <p className="text-sm text-slate-500">
          Peer{" "}
          <span className="font-semibold text-indigo-600">{senderNickname || sender}</span>{" "}
          wants to connect
        </p>
